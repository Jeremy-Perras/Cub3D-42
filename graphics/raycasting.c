...
static void	ft_distinit(t_data *data)
{
	...
	{
		data->ray.stepx = -1;
		data->ray.sidedistx = (data->player.position.x
				- (double) data->ray.mapx) * data->ray.deltadistx;
	}
	else
	{
		data->ray.stepx = 1;
		data->ray.sidedistx = ((double)data->ray.mapx + 1
				- data->player.position.x) * data->ray.deltadistx;
	}
	if (data->ray.diry < 0)
	{
		data->ray.stepy = -1;
		data->ray.sidedisty = (data->player.position.y
				- (double) data->ray.mapy) * data->ray.deltadisty;
	}
	else
	{
		data->ray.stepy = 1;
		data->ray.sidedisty = ((double)data->ray.mapy + 1
				- data->player.position.y) * data->ray.deltadisty;
	}
}

void	ft_wall(t_data *data)
{
	if ((data->ray.sidedistx >= 0
			|| data->ray.sidedisty <= 0)
		&& data->ray.sidedistx < data->ray.sidedisty)
	{
...
void	ft_raycasting(t_data *data)
{
	...
	while (pix < WIDTH)
	{
		...
		walltext = ft_choose_wall(data->ray.side,
				data->ray.dirx, data->ray.diry);
		ft_distwall(data);
		ft_draw(data, pix, walltext);
		pix++;
	}
	mlx_put_image_to_window(data->mlx, data->win.ref,
		data->image[20].image, 0, 0);
}
